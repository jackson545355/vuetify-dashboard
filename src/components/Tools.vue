    ...
    <v-card rounded="lg">
      ...
      <v-card-text>
        <v-form ref="form" v-model="valid">
          <v-row>
            <v-col cols="12"
              ><h4>Logo <span class="redstar">*</span></h4></v-col
            >
            <v-col cols="12" sm="3">
              <v-img
            ...
            <v-col cols="12"
              ><h4>Tool name <span class="redstar">*</span></h4></v-col
            >
            <v-col cols="12">
              <v-text-field
                v-model="toolName"
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          variant="elevated"
          :disabled="!valid"
          @click="editMode ? updateTool() : saveTool()"
...
export default {
  ...
  methods: {
    ...
    saveTool() {
      ...
      formData.append("logoTool", this.logoFile);
      axios
        .post("http://localhost:4000/master-items/tool", formData, {
          headers: {
    ...
    updateTool() {
      ...
      //   this.resetForm();
      const formData = new FormData();
      this.editItemData.id && formData.append("toolID", this.editItemData.id);
      this.toolName &&formData.append("toolName", this.toolName);
      this.logoFile && formData.append("logoTool", this.logoFile);
      axios
        .put(`http://localhost:4000/master-items/tool`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
    ...
    deleteItem(item) {
      ...
      //   }
      axios
        .delete(`http://localhost:4000/master-items/tool/${item.id}`)
        .then((response) => {
          if (response.data.status) {
            this.deleteItem(this.itemToDelete);
            this.snackbar = true;
    ...
    fetchData() {
      axios
        .get("http://localhost:4000/master-items/tools")
        .then((response) => {
          if (response.data.status) {
            this.items = response.data.data.map((tool) => ({
              name: tool.toolName,
              id: tool.toolId,
              logo: tool.logoTool,
            }));
